...
class QACacheManager:
    ...
    Each wiki gets its own FAISS index stored as:
      ...
      {cache_dir}/{wiki_id}.qa.ready — consistency marker (written last on save)

    Uses raw FAISS IndexFlatIP with L2-normalized embeddings (cosine similarity).
    In-memory indexes are LRU-evicted when the number of loaded wikis exceeds
    ``max_wikis`` (evicted indexes are reloaded from disk on next access).
    """

    def __init__(self, cache_dir: str, embeddings: Embeddings, max_wikis: int = 50) -> None:
        self._cache_dir = cache_dir
        self._embeddings = embeddings
        self._max_wikis = max_wikis
        self._indexes: OrderedDict[str, faiss.IndexFlatIP] = OrderedDict()
        self._qa_ids: OrderedDict[str, list[str]] = OrderedDict()
    ...
    def _ready_path(self, wiki_id: str) -> str:
        return os.path.join(self._cache_dir, f"{wiki_id}.qa.ready")

    # -- LRU helpers ----------------------------------------------------------

    def _touch(self, wiki_id: str) -> None:
        """Mark wiki as recently used (move to end of OrderedDict)."""
        if wiki_id in self._indexes:
            self._indexes.move_to_end(wiki_id)
        if wiki_id in self._qa_ids:
            self._qa_ids.move_to_end(wiki_id)

    def _evict_lru(self) -> None:
        """Evict least-recently-used in-memory indexes when over capacity."""
        while len(self._indexes) > self._max_wikis:
            evicted_id, _ = self._indexes.popitem(last=False)
            self._qa_ids.pop(evicted_id, None)
            logger.info(
                "QA index evicted for wiki %s (LRU, capacity %d)",
                evicted_id,
                self._max_wikis,
            )
    ...
    def _load_index(self, wiki_id: str) -> bool:
        ...
        ready_path = self._ready_path(wiki_id)
        if not os.path.exists(index_path) or not os.path.exists(ids_path):
            return False
        # Ready marker must exist — its absence means a save was interrupted
        if not os.path.exists(ready_path):
            logger.warning(
                "QA index ready marker missing for wiki %s — save was interrupted",
                wiki_id,
            )
            return False
        try:
    ...
    def check_needs_rebuild(self, wiki_id: str) -> bool:
        ...
        ids_path = self._ids_path(wiki_id)
        ready_path = self._ready_path(wiki_id)
        # Files exist but load failed (corrupt, mismatched, or missing ready marker)
        return os.path.exists(index_path) or os.path.exists(ids_path) or os.path.exists(ready_path)

    def _save_index(self, wiki_id: str) -> None:
        """Persist index + IDs with a ready marker for crash safety.

        1. Remove ready marker (invalidates current on-disk state)
        2. Write both files via temp-then-rename
        3. Write ready marker (commits the new state)

        If a crash occurs before step 3, the missing marker causes the next
        load to fail, triggering a rebuild from the DB (source of truth).
        """
        ...
        try:
            # Step 1: Invalidate current state
            ...
            # Step 3: Commit — mark files as consistent
    ...
    def delete_index(self, wiki_id: str) -> bool:
        ...
        deleted = False
        for path in (self._index_path(wiki_id), self._ids_path(wiki_id), self._ready_path(wiki_id)):
