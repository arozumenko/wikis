...
class OptimizedWikiGenerationAgent:
    ...
    def generate_wiki_structure(self, state: WikiState, config: RunnableConfig) -> dict[str, Any]:
        ...
        try:
            ...
            if self.progress_callback:
                try:
                    self.progress_callback("planning", 0.28, "Planning wiki structure...")
                except Exception:
                    pass

            # Unified pipeline is the only path (#242).
            repository_files = self._get_repository_file_paths()
            result = self._generate_wiki_structure(
            ...
            if result is not None:
                logger.info(
                    ...
                    len(result.get("wiki_pages", [])),
                )
                return result
            raise RuntimeError("Unified pipeline returned no result")
    ...
    def _generate_wiki_structure(
        ...
        This is the sole structure-planning path (#242). It replaces the
        former graph-first, deepagents, and cluster-based paths.
