...
class SymbolInfo:
    ...
    def __init__(self, name: str, node_type: str, start_line: int, end_line: int, file_path: str, language: str):
        self.name = name
...
class GraphAwareCodeSplitter:
    ...
    def split_repository(
        self, repo_path: str, file_patterns: list[str] | None = None
    ) -> tuple[list[Document], list[Document], nx.DiGraph]:
        """
        ...
        for root, dirs, files in os.walk(repo_path):
            ...
            dirs[:] = [
                ...
                for d in dirs
                if not d.startswith(".")
                and d not in {"node_modules", "__pycache__", "build", "dist", "target", "vendor"}
            ]
        ...
        logger.info(
            f"Built code_graph with {self.combined_graph.number_of_nodes()} nodes and {self.combined_graph.number_of_edges()} edges"
        )
    ...
    def _process_text_file(self, file_path: str, rel_path: str) -> list[Document]:
        ...
        try:
            ...
            if Path(file_path).suffix.lower() in {".md", ".markdown"}:
                ...
                return [
                    Document(
                        page_content=content, metadata={"source": rel_path, "file_type": "text", "chunk_type": "text"}
                    )
                ]
    ...
    def _process_with_regex(self, content: str, file_path: str, language: str) -> list[Document]:
        ...
        patterns = {
            "python": [r"^def\s+(\w+)\s*\(", r"^class\s+(\w+)\s*[:(\[]", r"^import\s+(\w+)", r"^from\s+(\w+)"],
            "javascript": [
                r"function\s+(\w+)\s*\(",
                r"class\s+(\w+)\s*{",
                r"const\s+(\w+)\s*=\s*\(",
    ...
    def _extract_symbol_info(self, node: "Node", lines: list[str], file_path: str, language: str) -> SymbolInfo | None:
    ...
    def _get_symbol_name(self, node: "Node", language: str) -> str | None:
    ...
    def _extract_docstring(self, node: "Node", lines: list[str], language: str) -> str | None:
    ...
    def _extract_return_type(self, node: "Node", language: str) -> str | None:
        ...
        for child_node in self._traverse_tree(node):
            ...
            if self._is_local_import(child_node, node, language):
                ...
                if child_node.type == "call":
                    ...
                    for child in child_node.children:
                        if child.type == "identifier" and child.text.decode("utf8") in [
                            "require",
                            "load",
                            "require_relative",
                        ]:
    ...
    def export_graph(self) -> dict:
        ...
        return {
            "nodes": [{"id": node_id, **data} for node_id, data in self.combined_graph.nodes(data=True)],
            "edges": [
    ...
    def process_repository(self, repo_path: str) -> tuple[list[Document], list[Document], nx.DiGraph]:
        ...
        logger.info(
            f"Built combined code_graph with {self.combined_graph.number_of_nodes()} nodes and {self.combined_graph.number_of_edges()} edges"
        )
        ...
        for symbol_key, symbol_info in self.symbol_table.items():
            ...
            for imported in all_imports:
                ...
                for target_key, target_info in self.symbol_table.items():
                    if (
                        target_info.language == symbol_info.language  # Same language check
                        and target_key != symbol_key
                        and (
                            target_info.name == imported
                            or target_info.file_path.endswith(f"{imported}.{self._get_extension(symbol_info.language)}")
                            or imported in target_key
                        )
                    ):
                ...
                if same_file_key in self.symbol_table:
                    ...
                    if "." in called:
                        ...
                        if len(parts) == 2:
                            ...
                            for target_key, target_info in self.symbol_table.items():
                                if (
                                    target_info.language == symbol_info.language  # Same language check
                                    and target_info.name == function_name
                                    and module_name in target_info.file_path
                                    and target_key != symbol_key
                                ):
                    ...
                    # Handle constructor calls (new ApiClient)
                        ...
                        for target_key, target_info in self.symbol_table.items():
                            if (
                                target_info.language == symbol_info.language  # Same language check
                                and target_info.name == class_name
                                and target_info.node_type
                                in ["class_declaration", "class_definition", "struct_item", "class"]
                                and target_key != symbol_key
                            ):
                                self.combined_graph.add_edge(symbol_key, target_key, relationship="calls")
                                break
                    ...
                    # Direct function calls across files (same language only)
                        ...
                        for target_key, target_info in self.symbol_table.items():
                            if (
                                target_info.language == symbol_info.language  # Same language check
                                and target_info.name == called
                                and target_info.file_path != symbol_info.file_path
                                and target_info.node_type
                                in [
                                    "function_definition",
                                    "function_declaration",
