"""
SQLite storage backend for Wikis.

This module is the canonical SQLite implementation of the storage protocol.
It keeps the historical ``UnifiedWikiDB`` class name as a compatibility alias,
but the implementation now lives under ``app.core.storage.sqlite`` where it
belongs alongside the other storage backends.
...
SqliteWikiStorage = UnifiedWikiDB
