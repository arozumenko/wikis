...
def resolve_alias_chain(graph, node_id: str, max_hops: int = 5) -> Optional[str]:
...
def augment_cpp_node(graph, node_id: str) -> Optional[AugmentedContent]:
...
def _find_defines_body_impl(
    graph, decl_node: str, decl_file: str
...
def augment_go_node(graph, node_id: str) -> Optional[AugmentedContent]:
    ...
    for method_node in graph.successors(node_id):
        ...
        method_file = (
            method_data.get('rel_path', '')
            or method_data.get('file_path', '')
        )
...
def augment_rust_node(graph, node_id: str) -> Optional[AugmentedContent]:
    """Augment a Rust struct/enum/trait with impl methods defined in other files.

    In Rust, ``impl`` blocks can live in separate files from the type they
    implement.  The parser links these cross-file methods via
    ``DEFINES(struct → method)`` edges with ``cross_file: True``.

    This is analogous to Go receiver-method augmentation and C++ header/impl
    splits.  Same-file methods are already visible in the surrounding source
    context and are **not** duplicated here.

    Returns an ``AugmentedContent`` if cross-file impl methods were found,
    else ``None``.
    """
    node_data = graph.nodes.get(node_id, {})
    if not node_data:
    ...
    # Collect impl methods via DEFINES edges
    methods_by_file: Dict[str, List[str]] = {}
    for method_node in graph.successors(node_id):
        if not has_relationship(graph, node_id, method_node, 'defines'):
            continue
        ...
        method_data = graph.nodes.get(method_node, {})
        method_type = method_data.get('symbol_type', '').lower()
        if method_type not in ('method', 'function', 'constructor'):
            continue

        # Only augment cross-file methods; same-file methods are already
        # visible in the source context surrounding the type definition.
        method_file = (
            method_data.get('rel_path', '')
            or method_data.get('file_path', '')
        )
        if not method_file or method_file == type_file:
            continue
        ...
        # Even without the annotation, different file paths are sufficient
        if not is_cross and method_file == type_file:
            continue
...
def _expand_class(graph, node_id: str, already_expanded: Set[str]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'inheritance'}, 'successors', limit=3
    ):
        _add(nid, f"base class via {rt}")

    # P0: Implementation (class → interfaces)
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'implementation'}, 'successors', limit=2
    ):
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'instantiates'}, 'successors', limit=2,
    ):
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'composition', 'aggregation'}, 'successors', limit=3,
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'alias_of'}, 'successors', limit=2
    ):
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'specializes'}, 'successors', limit=2
    ):
        _add(nid, f"template base via {rt}")
        for arg_nid, arg_rt in get_neighbors_by_relationship(
            graph, nid, {'references'}, 'successors', limit=2,
            type_filter=CLASS_LIKE_TYPES | frozenset({'type_alias'}),
        ):
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'references'}, 'successors', limit=2,
        type_filter=CLASS_LIKE_TYPES | frozenset({'type_alias'}),
    ):
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'inheritance'}, 'predecessors', limit=3,
    ...
    if node_sym in ('interface', 'trait'):
        for nid, rt in get_neighbors_by_relationship(
            graph, node_id, {'implementation'}, 'predecessors', limit=3,
            type_filter=CLASS_LIKE_TYPES,
        ):
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'composition', 'aggregation'}, 'predecessors', limit=2,
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'creates', 'instantiates'}, 'predecessors', limit=2,
        type_filter=CLASS_LIKE_TYPES | frozenset({'function', 'method'}),
    ):
        if rt == 'instantiates':
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'references'}, 'predecessors', limit=2,
        type_filter=CLASS_LIKE_TYPES | frozenset({'function', 'method'}),
    ):
...
def _expand_function(graph, node_id: str, already_expanded: Set[str]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'creates', 'instantiates'}, 'successors', limit=3,
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'calls'}, 'successors', limit=2,
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'calls'}, 'predecessors', limit=3,
...
def _expand_constant(graph, node_id: str, already_expanded: Set[str]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    # P2: References (type)
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'references'}, 'successors', limit=1,
        type_filter=CLASS_LIKE_TYPES | frozenset({'type_alias'}),
    ):
...
def _expand_macro(graph, node_id: str, already_expanded: Set[str]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'references'}, 'successors', limit=3,
    ...
    for nid, rt in get_neighbors_by_relationship(
        graph, node_id, {'references', 'calls'}, 'predecessors', limit=3,
    ...
    *,
    ...
    for node_id in matched_nodes:
        if total_added >= global_cap:
            logger.debug(
                "[SMART_EXPAND] Global cap %d reached, stopping", global_cap
            )
            break
        ...
        # --- Rust augmentation (P0, in-place, no budget cost) ---
        # Rust impl blocks can live in separate files from their type.
        # Augment the type content with cross-file impl methods so the
        # LLM sees the full API surface — analogous to Go receiver methods.
        if lang == 'rust' and sym_type in ('struct', 'class', 'enum', 'trait'):
            aug = augment_rust_node(graph, node_id)
            if aug:
                result.augmentations[node_id] = aug
                logger.debug(
                    "[SMART_EXPAND] Rust %s %s augmented with impl methods from %s",
                    sym_type, node_id, aug.impl_file,
                )
