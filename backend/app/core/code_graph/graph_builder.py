...
_BASIC_AST_TYPE_MAP: dict[str, str] = {
    # Go
    "function_declaration": "function",  # shared: Go, Kotlin
    "method_declaration": "method",  # shared: Go, C#
    "struct_item": "struct",  # shared: Go, Rust; refined by _classify_go_type_declaration
    ...
    # C#
    "class_declaration": "class",  # shared: C#, Kotlin
    "constructor_declaration": "constructor",
    "interface_declaration": "interface",
    "enum_declaration": "enum",
    "struct_declaration": "struct",
    "property_declaration": "constant",  # shared: C#, Kotlin
    "using_directive": "import",
    # Ruby
    "method": "method",  # also identity mapping
    "singleton_method": "method",
    "class": "class",  # also identity mapping
    "module": "module",  # also identity mapping
...
class UnifiedAnalysis:
    ...
    documents_iter: Iterable[Document] | None = None  # Optional streaming iterator
    unified_graph: nx.MultiDiGraph | None = None  # Multi-tier code_graph with preserved relationships
...
class EnhancedUnifiedGraphBuilder:
    ...
    def __init__(self, max_workers: int = 4, debug_mode: bool = False):
        ...
        supported_basic_languages = ["csharp", "ruby", "kotlin", "scala"]
        ...
        self.last_analysis: UnifiedAnalysis | None = None
        ...
        logger.info("Enhanced Unified Graph Builder initialized:")
        ...
        logger.info("  Languages without symbol extraction: C, C++, PHP (use regex fallback)")
    ...
    def _is_package_or_namespace_parent(
        self, parent_symbol: str, parse_results: dict[str, Union["ParseResult", "BasicParseResult"]]
    ) -> bool:
        """
        ...
        for _file_path, result in parse_results.items():
            for symbol in result.symbols:
                # Get symbol type string consistently
                if hasattr(result, "parse_level") and result.parse_level == "comprehensive":
                    symbol_type_str = (
                        symbol.symbol_type.value if hasattr(symbol.symbol_type, "value") else str(symbol.symbol_type)
                    )
                else:
                ...
                if symbol.name == parent_symbol and symbol_type_str.lower() in {
                    "namespace",
                    "module",
                    "package",
                    "crate",
        ...
        if "." in parent_symbol:
            ...
            looks_like_package = all(
                part.islower() or part.isdigit() or "_" in part or part.replace("_", "").islower() for part in parts
            )
        ...
        class_indicators = {
            ...
            "has_type_suffix": any(
                suffix in parent_symbol.lower() for suffix in ["class", "interface", "struct", "enum", "trait", "impl"]
            ),
            # CamelCase pattern (common in OOP languages)
            "is_camel_case": (
                parent_symbol
                and parent_symbol[0].isupper()
                and any(c.isupper() for c in parent_symbol[1:])
                and "." not in parent_symbol
                and "_" not in parent_symbol
            ),
        ...
        if (
            parent_symbol.islower()
            and len(parent_symbol) <= 20  # Reasonable package name length
            and "_" not in parent_symbol
            and not any(c.isupper() for c in parent_symbol)
        ):
            return True  # Likely Go package

        # Rust: module names are typically lowercase with underscores
        if parent_symbol.islower() and ("_" in parent_symbol or parent_symbol.isalpha()) and len(parent_symbol) <= 30:
        ...
        with resource_monitor("graph_builder.parse_by_language", logger):
            for language, file_paths in files_by_language.items():
                ...
                if this and getattr(this, "module", None):
                    tier = (
                        "rich"
                        if language in self.rich_parsers
                        else ("basic" if language in self.basic_languages else "unsupported")
                    )
                    this.module.invocation_thinking(
                ...
                if language == "documentation":
                    ...
                    logger.warning(
                        f"Language {language} has tree-sitter parser but no symbol extraction implementation yet"
                    )
    ...
    def _discover_files_by_language(
        self, repo_path: str, include_patterns: list[str] | None = None, exclude_patterns: list[str] | None = None
    ) -> dict[str, list[str]]:
    ...
    def _parse_with_rich_parser(self, language: str, file_paths: list[str]) -> dict[str, ParseResult]:
        ...
        if hasattr(parser, "parse_multiple_files"):
            ...
            logger.warning(
                f"Rich parser for {language} missing parse_multiple_files method, falling back to individual parsing"
            )
            results = {}
    ...
    def _parse_with_basic_parser_single_file(
        self, file_path: str, language: str, repo_root: str = ""
    ...
    def _build_multi_tier_graph(self, parse_results: dict[str, ParseResult | BasicParseResult]) -> nx.MultiDiGraph:
        ...
        basic_group_count = sum(
            1 for language in self.basic_languages if any(res.language == language for res in basic_results.values())
        )
    ...
    def _build_graph_indexes(self, graph: nx.MultiDiGraph) -> None:
        ...
        for _key, nodes in graph._name_index.items():
            nodes.sort(key=_priority, reverse=True)

        for _key, nodes in graph._suffix_index.items():
            nodes.sort(key=_priority, reverse=True)

    def _build_comprehensive_language_graph(
        self, parse_results: dict[str, ParseResult], language: str
    ) -> nx.MultiDiGraph:
        ...
        self._build_graph_sync(graph, parse_results, language, symbol_analysis)

        build_time = time.time() - start_time
        logger.info(
            f"Comprehensive {language} code_graph built in {build_time:.2f}s: {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges"
        )
    ...
    def _analyze_symbol_loss(self, parse_results: dict[str, ParseResult], language: str = "python") -> dict[str, Any]:
        ...
        for file_path, result in parse_results.items():
            ...
            for symbol in result.symbols:
                ...
                if node_id not in processed_symbols:
                    ...
                    symbol_definitions[symbol_key].append(
                        (
                            file_path,
        ...
        logger.info("SYMBOL LOSS ANALYSIS:")
        ...
        if analysis["duplicate_definitions"]:
            logger.warning(
                f"Legitimate duplicate definitions found (same symbol multiple times in same file): {list(analysis['duplicate_definitions'].items())[:5]}"
            )
    ...
    def _build_graph_sync(
        ...
        logger.info("Phase 1: Adding symbols as nodes...")
    ...
    def _add_symbols_batch(
        ...
        logger.info(
            f"SYMBOL SUMMARY: Expected {total_symbols_expected}, Added {final_symbols}, Loss: {total_symbols_expected - final_symbols}"
        )
    ...
    def _add_relationships_bulk(
        ...
        for file_path, result in parse_results.items():
            ...
            if processed_files % 500 == 0:
                ...
                logger.info(
                    f"Prepared relationships from {processed_files}/{total_files} files "
        ...
        if relationships_added != final_edges:
            ...
            logger.warning("   This may indicate duplicate edges or MultiDiGraph consolidation")

    def _resolve_source_node_sync(
        self,
        source_symbol: str,
        language: str,
        file_name: str,
    ...
    def _generate_symbol_chunks(self, parse_results: dict[str, ParseResult | BasicParseResult]) -> list[Document]:
        """
        ...
        for file_path, result in parse_results.items():
            ...
            if processed_files % 500 == 0 or processed_files == total_files:
                elapsed = time.time() - start_time
                logger.info(
                    f"Processing symbols: {processed_files}/{total_files} files, "
        ...
        if SEPARATE_DOC_INDEX:
            ...
            logger.info(
                f"Symbol chunk generation complete: {len(documents)} documents created "
    ...
    def _iter_symbol_chunks(self, parse_results: dict[str, ParseResult | BasicParseResult]):
    ...
    def _iter_symbol_chunks_with_cleanup(self, parse_results: dict[str, ParseResult | BasicParseResult]):
        """Stream symbol chunks and clear parse results after consumption."""
        try:
            yield from self._iter_symbol_chunks(parse_results)
        finally:
            try:
                parse_results.clear()
            except Exception:  # noqa: S110
    ...
    def _detect_cross_language_relationships(
        self, files_by_language: dict[str, list[str]], parse_results: dict[str, ParseResult | BasicParseResult]
    ) -> list[Any]:
    ...
    def _generate_language_stats(self, parse_results: dict[str, ParseResult | BasicParseResult]) -> dict[str, Any]:
