...
class TypeScriptEnhancedParser(BaseParser):
    ...
    def extract_symbols(self, ast_node: Any, file_path: str) -> list[Symbol]:
        ...
        class SymbolExtractor:
            ...
            def visit_type_alias_declaration(self, node):
                ...
                type_node = self.parser._find_child_by_type(
                    node, ["union_type", "intersection_type", "object_type", "type_identifier"]
                )
            ...
            def visit_enum_declaration(self, node):
                ...
                # Metadata for language-specific features
                metadata = {"is_enum": True, "is_const_enum": is_const}
                if is_exported:
                    metadata["is_exported"] = True
            ...
            def visit_decorator(self, node):
                ...
                if decorator_name:
                    ...
                    self.current_decorators.append({"name": decorator_name, "range": self.parser._node_to_range(node)})
            ...
            def _get_access_modifier(self, node) -> str | None:
            ...
            def _parse_class_heritage(self, heritage_node):
                ...
                for child in heritage_node.children:
                    if child.type == "extends_clause":
                        ...
                        type_node = self.parser._find_child_by_type(
                            child, ["identifier", "type_identifier", "generic_type", "member_expression"]
                        )
                        if type_node:
            ...
            def _extract_parameter_info(self, param_node) -> dict | None:
            ...
            def _extract_type_annotation(self, node) -> str | None:
                ...
                for child in type_arguments.children:
                    if child.type in ["object_type", "type_identifier", "type_reference"]:
                        if child.type == "object_type":
                            ...
                            prop_types.append({"name": "props", "type": props_type_name, "is_optional": False})
                        break
            ...
            def _extract_object_type_properties(self, object_type_node) -> list[dict]:
                ...
                for child in object_type_node.children:
                    if child.type == "property_signature":
                        ...
                        if prop_type:
                            properties.append({"name": prop_name, "type": prop_type, "is_optional": is_optional})
            ...
            def _extract_variable_from_declarator(self, node, kind="const"):
                ...
                symbol = Symbol(
                    ...
                    metadata={"kind": kind, "is_exported": is_exported},
                )
                self.symbols.append(symbol)

            def _extract_arrow_function_as_symbol(
                self, declarator_node, arrow_func_node, name, full_name, parent_symbol
            ):
            ...
            def _extract_field_type_relationship(self, field_type, field_symbol, field_name, node):
                ...
                base_type = (
                    field_type.replace("?", "").replace("[]", "").replace("undefined", "").replace("null", "").strip()
                )
                            ...
                            for arg_child in type_args.children:
                                if arg_child.type not in ["<", ">", ","]:
                                    self._extract_type_references_recursive(
                                        arg_child, self.current_symbol, "constructor_generic_arg"
                                    )
                        break
                    ...
                    if base:
                        ...
                        if base_name and base_name not in self.ts_keywords and base_name not in builtin_generics:
                            alias_rel = Relationship(
                                ...
                                annotations={
                                    "alias_name": name,
                                    "aliased_type": self.parser._get_node_text(type_value),
                                },
                            )
                            self.relationships.append(alias_rel)
                    ...
                    if type_args:
                        for child in type_args.children:
                            if child.type not in ["<", ">", ","]:
                                self._extract_type_references_recursive(
                                    child, source_symbol, f"{reference_type}_generic_arg"
                                )
    ...
    def parse_multiple_files(self, file_paths: list[str], max_workers: int = 4) -> dict[str, ParseResult]:
        ...
        logger.info(
            f"TypeScript parsing complete: {len(results)}/{total} files processed, {successful_parses} successful"
        )
        return results
