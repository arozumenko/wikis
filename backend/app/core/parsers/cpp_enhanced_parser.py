...
def _parse_single_cpp_file(file_path: str) -> tuple[str, ParseResult]:
    ...
    try:
        ...
        return file_path, ParseResult(
            file_path=file_path, language="cpp", symbols=[], relationships=[], errors=[str(e)]
        )
...
class CppEnhancedParser(BaseParser):
    ...
    def parse_file(self, file_path: str | Path, content: str | None = None) -> ParseResult:
        ...
        try:
            ...
            # Calculate file hash
            file_hash = hashlib.md5(content.encode()).hexdigest()  # noqa: S324 — content fingerprint for cache key, not cryptographic use
        ...
        class SymbolExtractor:
            ...
            def visit_alias_declaration(self, node):
                ...
                if aliased_type:
                    ...
                    for base_type, _is_ptr_or_ref in udt_types:
                        ...
                        relationship = Relationship(
                            ...
                            annotations={"alias_name": name, "aliased_type": aliased_type},
                        )
            ...
            def visit_type_definition(self, node):
                ...
                if base_type:
                    ...
                    for udt_name, _is_ptr_or_ref in udt_types:
                        relationship = Relationship(
                            source_symbol=full_name,
                            ...
                            source_file=file_path,
                            source_range=self.parser._node_to_range(node),
                            annotations={"alias_name": name, "base_type": base_type},
                        )
                        self.relationships.append(relationship)
            ...
            def _handle_template_method_declaration(self, decl_node):
                ...
                if self.current_template_params:
                    symbol.metadata = {"is_template": True, "template_params": self.current_template_params}
            ...
            def _extract_field_type_relationship(self, field_type, field_symbol, field_name, node):
                ...
                for base_type, is_pointer_or_ref in base_types:
                    ...
                    relationship = Relationship(
                        ...
                        source_file=file_path,
                        source_range=self.parser._node_to_range(node),
                        annotations={"field_name": field_name, "field_type": field_type},
                    )
                    self.relationships.append(relationship)
                ...
                if return_type:
                    ...
                    for base_type, _ in return_types:
                        relationship = Relationship(
                            ...
                            source_file=file_path,
                            source_range=self.parser._node_to_range(node),
                            annotations={"usage": "return_type", "return_type": return_type},
                        )
                        self.relationships.append(relationship)
                    ...
                    if param_type:
                        ...
                        for base_type, _ in param_types_extracted:
                            relationship = Relationship(
                                ...
                                source_file=file_path,
                                source_range=self.parser._node_to_range(node),
                                annotations={"usage": "parameter_type", "parameter_type": param_type},
                            )
                            self.relationships.append(relationship)
            ...
            def _extract_user_defined_types_from_field(self, field_type, _depth=0):
                ...
                PRIMITIVE_TYPES = {
                    ...
                    "ssize_t",
                    "string",
                    "wstring",  # std::string treated as standard
                    "auto",
                    "decltype",
                    "char16_t",
                    "char32_t",
                ...
                base_type = (
                    field_type.replace("const", "").replace("volatile", "").replace("*", "").replace("&", "").strip()
                )
                ...
                base_type.split("::")[-1] if "::" in base_type and "<" not in base_type else base_type
    ...
    class RelationshipExtractor:
        ...
        def _find_symbol_by_signature(self, full_name, name, parameter_types, is_const=False):
            ...
            for candidate in candidates:
                param_match = hasattr(candidate, "parameter_types") and candidate.parameter_types == parameter_types
                const_match = candidate.metadata and candidate.metadata.get("is_const_method", False) == is_const
        ...
        def _extract_type_from_param(self, param_node):
            ...
            for child in param_node.children:
                if child.type == "type_qualifier":
                    ...
                    self.parser._get_node_text(child)
        ...
        def visit(self, node):
            ...
            if not has_custom_visitor or node.type not in [
                "class_specifier",
                "struct_specifier",
                "namespace_definition",
                "declaration",
        ...
        def visit_call_expression(self, node):
            ...
            if function_node.type == "identifier":
                ...
                if template_func_node:
                    ...
                    for child in function_node.children:
                        if child.type in ("namespace_identifier", "identifier", "type_identifier"):
                            ...
                            parts.extend(
                                p for p in _collect_qualified_parts_from_node(child) if child is not function_node
                            )
                        elif child.type == "template_function":
                ...
                if (
                    template_func_node
                    and called_name
                    and ("make_unique" in called_name or "make_shared" in called_name)
                ):
            ...
            if called_name and called_name not in self.cpp_keywords:
                ...
                if is_constructor:
                    ...
                    relationship = Relationship(
                        ...
                        source_file=self.file_path,
                        source_range=self.parser._node_to_range(node),
                        annotations={"creation_type": "stack", "creation_context": "statement"},
                    )
                else:
        ...
        def visit_template_type(self, node):
            ...
            while parent:
                if parent.type == "base_class_clause":
                    in_base_clause = True
                    break
                if parent.type in ("class_specifier", "struct_specifier", "function_definition", "translation_unit"):
                    break
        ...
        def _extract_field_access(self, node):
            ...
            relationship = Relationship(
                ...
                source_file=self.file_path,
                source_range=self.parser._node_to_range(node),
                annotations={"reference_type": "field_access", "field_name": field_name},
            )
            self.relationships.append(relationship)
                    ...
                    for symbol_list in self.symbol_by_name.get(object_name, []):
                        for symbol in symbol_list if isinstance(symbol_list, list) else [symbol_list]:
                            if symbol.symbol_type == SymbolType.PARAMETER:
                                if (
                                    symbol.parent_symbol == self.current_symbol.full_name
                                    or symbol.parent_symbol == self.current_symbol.name
                                ):
                    ...
                    for i, sibling in enumerate(function_node.children):
                        if sibling == child:
                            if i > 0:
                                ...
                                if prev.type in ["type_identifier", "qualified_identifier", "template_type"]:
        ...
        def visit_new_expression(self, node):
            ...
            if type_name and type_name not in self.cpp_keywords:
                ...
                relationship = Relationship(
                    ...
                    source_file=self.file_path,
                    source_range=self.parser._node_to_range(node),
                    annotations={"creation_type": creation_type, "creation_context": "statement"},
                )
                self.relationships.append(relationship)
    ...
    def _find_child_by_type(self, node, child_type: str | list[str]):
    ...
    def parse_multiple_files(self, file_paths: list[str], max_workers: int = 4) -> dict[str, ParseResult]:
        ...
        try:
            ...
            logger.info("Pass 3 complete: Cross-file relationships enhanced")
        except Exception as e:
