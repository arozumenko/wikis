...
def _parse_single_python_file(file_path: str) -> tuple[str, ParseResult]:
    ...
    try:
        ...
        return file_path, ParseResult(file_path=file_path, language="python", symbols=[], relationships=[])
...
class PythonParser(BaseParser):
    ...
    def extract_symbols(self, ast_node: Any, file_path: str) -> list[Symbol]:
        ...
        class SymbolExtractor(ast.NodeVisitor):
            ...
            def visit_ClassDef(self, node):
                ...
                # Extract decorators
                [self.parser._get_decorator_name(dec) for dec in node.decorator_list]
    ...
    def extract_relationships(
        self, ast_node: Any, symbols: list[Symbol], file_path: str
    ) -> tuple[list[Relationship], list[Symbol]]:
        """
        ...
        for symbol in symbols:
            ...
            for member in members:
                ...
                relationship = Relationship(
                    ...
                    source_file=file_path,
                    source_range=member.source_code_location
                    if hasattr(member, "source_code_location") and member.source_code_location
    ...
    def _extract_fields_and_composition(
        self, symbols: list[Symbol], file_path: str, ast_node: ast.Module
        ...
        init_methods = [
            s for s in symbols if s.symbol_type in (SymbolType.METHOD, SymbolType.CONSTRUCTOR) and s.name == "__init__"
        ]
            ...
            for node in ast.walk(init_node):
                ...
                if field_name:
                    ...
                    if field_type and field_type not in builtin_types:
                        ...
                        rel_type = (
                            RelationshipType.AGGREGATION
                            if (is_optional_wrapper or is_none)
                            else RelationshipType.COMPOSITION
                        )
        ...
        for class_symbol in class_index.values():
            ...
            for node in class_node.body:
                if isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name):
                    ...
                    # Create COMPOSITION or AGGREGATION relationship for non-builtin types
                    if field_type and field_type not in builtin_types:
                        annotations = {"field_name": field_name, "class_variable": True}
                        if module_prefix:
                            annotations["module_prefix"] = module_prefix
    ...
    def _build_function_signature(self, node: ast.FunctionDef, return_type: str | None) -> str:
    ...
    def _extract_type_name_from_annotation(self, annotation: ast.AST) -> str | None:
        """
    ...
    def _extract_type_references_recursive(
        ...
        if isinstance(annotation, ast.Name):
            ...
            self._extract_type_references_recursive(
                annotation.left, source_symbol, file_path, reference_type, relationships
            )
            self._extract_type_references_recursive(
                annotation.right, source_symbol, file_path, reference_type, relationships
            )
    ...
    def _extract_module_docstring(self, ast_tree: ast.Module) -> str | None:
    ...
    def parse_multiple_files(self, file_paths: list[str], max_workers: int = 4) -> dict[str, ParseResult]:
        ...
        logger.info(
            f"Global class registry built: {len(self._global_class_locations)} classes, "
        ...
        for file_path, result in results.items():
            if result.symbols:  # Only process files that parsed successfully
                ...
                try:
                    ...
                    new_relationships, new_field_symbols = self.extract_relationships(
                        ast_tree, result.symbols, file_path
                    )
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for future in concurrent.futures.as_completed(future_to_file):
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to parse Python file {file_path}: {type(e).__name__}: {e}", exc_info=True
                            )
                            results[file_path] = ParseResult(
                                file_path=file_path,
                                language="python",
    ...
    def _extract_global_symbols(self, file_path: str, result: ParseResult):
        ...
        for local_name, full_name in imports.items():
            ...
            if "." in full_name:
                ...
                for _other_file_path in [
                    fp for fp in self._global_class_locations.values() if Path(fp).stem == module_name
                ]:
