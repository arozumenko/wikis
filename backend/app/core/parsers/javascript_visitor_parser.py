...
class ImportBinding:
    ...
    imported: str | None = None  # original exported name (None for default)
...
class JavaScriptVisitorParser(BaseParser):
    ...
    def __init__(self):
        ...
        self._jsx_usages: list[tuple[str, Range, str | None]] = []  # (raw_tag, range, enclosing_symbol)
    ...
    def parse_file(self, file_path: str | Path, content: str | None = None) -> ParseResult:
        ...
        try:
            ...
            return ParseResult(
                file_path=file_path, language="javascript", symbols=[], relationships=[], errors=[str(e)]
            )
    ...
    def parse_multiple_files(self, file_paths: list[str], max_workers: int = 4) -> dict[str, ParseResult]:
        ...
        if num_batches > 1:
            logger.info(
                f"Processing {total} JavaScript files in {num_batches} batch(es) of up to {batch_size} files each"
            )
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for future in concurrent.futures.as_completed(future_to_file):
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to parse JavaScript file {file_path}: {type(e).__name__}: {e}", exc_info=True
                            )
                            results[file_path] = ParseResult(
                                file_path=file_path,
                                language="javascript",
        ...
        for _fp, entries in per_file_exports.items():
        ...
        for fp, result in results.items():
            self._enhance_file_relationships(
                fp,
                result,
                per_file_imports[fp],
                per_file_exports,
                global_exports,
                per_file_method_index,
                results,
    ...
    _JSDOC_PARAM_RE = __import__("re").compile(r"@param\s+\{([^}]+)\}\s+(\w+)")
    _JSDOC_RETURNS_RE = __import__("re").compile(r"@returns?\s+\{([^}]+)\}")
    _JSDOC_TYPE_RE = __import__("re").compile(r"@type\s+\{([^}]+)\}")

    def _get_preceding_jsdoc(self, node: Node) -> str | None:
    ...
    def _emit_jsdoc_references(self, jsdoc_data: dict[str, Any], source_symbol: str, source_range: Range):
        ...
        for ic_child in import_clause.children:
            ...
            if t == "identifier":  # default import
                ...
                self._import_bindings[local] = ImportBinding(
                    local=local, source_path=source_path, imported=None, is_default=True
                )
                self._relationships.append(
                    Relationship(
                        source_symbol=self._module_symbol_name,
                ...
                if ident:
                    ...
                    self._import_bindings[local] = ImportBinding(
                        local=local, source_path=source_path, imported="*", is_namespace=True
                    )
                    self._relationships.append(
                        Relationship(
                            source_symbol=self._module_symbol_name,
                ...
                for spec in ic_child.children:
                    if spec.type == "import_specifier":
                        ...
                        if local:
                            self._import_bindings[local] = ImportBinding(
                                local=local, source_path=source_path, imported=imported, is_default=False
                            )
                            self._relationships.append(
                                Relationship(
                                    source_symbol=self._module_symbol_name,
    ...
    def _handle_field_definition(self, node: Node):
        ...
        name_node = next(
            (c for c in node.children if c.type in ("property_identifier", "private_property_identifier")), None
        )
        if not name_node:
            return
    ...
    def _handle_jsx_element(self, node: Node):
        ...
        name_node = next(
            (
                c
                for c in node.children
                if c.type in ("identifier", "jsx_identifier", "jsx_member_expression", "member_expression")
            ),
            None,
        )
        if not name_node:
    ...
    # Helper extraction
    # ------------------------------------------------------------------
    def _extract_callee_name(self, node: Node) -> str | None:
    ...
    def _try_register_require_bindings(self, declarator_node: Node):
        ...
        """
        obj_pattern = next((c for c in declarator_node.children if c.type == "object_pattern"), None)
        call_expr = next((c for c in declarator_node.children if c.type == "call_expression"), None)
        ...
        args_node = next((c for c in call_expr.children if c.type == "arguments"), None)
        if not args_node:
            return
        ...
        for p_child in obj_pattern.children:
            if p_child.type in ("shorthand_property_identifier", "shorthand_property_identifier_pattern"):
                ...
                key_node = next((c for c in p_child.children if c.type in ("property_identifier", "identifier")), None)
                value_node = next(
                    (
                        c
                        for c in p_child.children
    ...
    def _extract_defines_relationships(self):
        ...
        for symbol in self._symbols:
            if symbol.symbol_type == SymbolType.CLASS:
                ...
                members = symbols_by_parent.get(class_name, []) + symbols_by_parent.get(full_class_name, [])
    ...
    # Enhancement
    # ------------------------------------------------------------------
    def _enhance_file_relationships(
        self,
        file_path: str,
        result: ParseResult,
        ...
        global_symbol_registry: dict[str, str] | None = None,
    ):
    ...
    def _resolve_import_path(
        self, from_file: str, source_path: str, per_file_exports: dict[str, list[ExportEntry]]
    ) -> str | None:
